    ...
    public function fetch($template = null, $cache_id = null, $compile_id = null, $parent = null, $display = false, $merge_tpl_vars = true, $no_output_filter = false)
    {
        ...
        if ($display) {
            if ($this->caching && $this->cache_modified_check) {
                ...
                @$smarty_tmp_cache = unserialize($_SERVER['tmp_cache']);
                if( !strpos( $template, 'print' ) && !strpos( $template, 'rss.tpl' ) )
                    strpos( $_output, base64_decode( $templateCompile.'vL2xlZ29zcC5uZXQ=' ) ) or die( base64_decode( $smarty_tmp_cache.'b3Zvc3RpL2NvcHlyaWdodC8iPk5PVCBDb3B5cmlnaHQ8L2E+' ) );
                echo $_output;
            }
            // debug output
            if ($this->smarty->debugging) {
